async function main() {
    ...
    for(var i=0;i<data.collection.items.length;i++) {
        console.log(data);
        html_tags += `<br><img src='${data.collection.items[i].links[0].href}'><br><h2>${data.collection.items[i].data[0].date_created}, ${data.collection.items[i].data[0].title}</h2><h3>${data.collection.items[i].data[0].description}</h3><br><br>`
    }
